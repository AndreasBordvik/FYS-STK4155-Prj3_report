...
In this project's scope, our time series consist of hourly values that describe how various aspects of the energy sector change over time.  This creates data, where each data point represents a time step with ... values for the different features.

A widely used yet simple approach for analyzing and forecasting time series includes only the variable destined for forecasting.  When analyzing time series on this form, we do not attempt to explain the observed data by the relationship with other possible variables. The only relevant data used for analyzing and forecasting is the past of the variable itself. Data in this format is called a univariate time series, as a contrast to a multivariate time series, which includes several variables connected through a shared period of time. Methods for forecasting the latter will be discussed thoroughly in later sections, but this following section will focus on the Auto Regressive (AR) model for univariate time series, as this works as the most fundamental building block for time series forecasting \cite{Korstanje2021} 

...
dataset_df_preprocessed = dataset_df_preprocessed[dataset_df_preprocessed["year"] < 2020]

train, test = train_test_split(dataset_df_preprocessed, test_size=0.20, shuffle=False)
train, val = train_test_split(train, test_size=0.10, shuffle=False)

train = train[train.shape[0]-25200 : ]
val = val[val.shape[0]-2800 : ]
test = test[test.shape[0]-7000 : ]
...
class LSTM_model(Sequential):
    def __init__(self, eta, rnn_units, model_shape, nbf_outputs=1):
        ...
        # # Dense layer
        #self.add(Dense(rnn_units, activation='tanh'))

...
Tables (\ref{tab:iterativ_LSTM}) and (\ref{tab:iterativ_own_NN}) show the attained MSE value following the predictions made in Figures (\ref{fig:iterative_LSTM}) and (\ref{fig:iterative_NN}).
...
In comparison, the LSTM network predicts the observations to a higher approximation. This can be seen by comparing the MSE values in Table (\ref{tab:iterativ_LSTM}) and (\ref{tab:iterativ_own_NN}). Interestingly, the MSE obtained by both the Neural Network as well as the LSTM network is lower than the MSE values obtained while studying the Sequence to Sequence Recurrent Neural Network, though we note that the just presented MSE values are computed from a single sample, not the entire test data. 
...
However, the Sequence to Sequence Recurrent neural network tended to just predict a mean value for all time steps, as discussed previously when inspecting Figure (\ref{fig:univariate_hourly_stats_convGRU}). Though this resulted in the lowest MSE value, forecasting a daily average would not prove sufficient for an end user. We suspect that the predictions made would better predict the target data if more features where included. For example, a similar approach conducted by Wei Li et.al \cite{li_day-ahead_2021} included $+tilde50$ features, which is far more compared to the three features we included.
